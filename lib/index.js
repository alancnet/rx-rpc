...
class RxRpc {
    constructor(options) {
        ...
        this.input.filter(x => x.hasOwnProperty('$$rxrpc'))
            .forEach(x => {
                ...
                switch (cmd) {
                    case 'next': stream && stream.onNext(this.unconvert(val)); break;
                    case 'error': stream && stream.onError(this.unconvert(val)); break;
                    case 'completed': stream && stream.onCompleted(); break;

                    case 'subscribe':
                        if (observable) {
                            this.subscriptions[id] = observable.subscribe(
                                next => this.output.onNext({$$rxrpc:['next', id, this.convert(next)]}),
                                error => this.output.onNext({$$rxrpc:['error', id, this.convert(error)]}),
                                completed => this.output.onNext({$$rxrpc:['completed', id]})
                            );
                        }
                        break;
                    case 'unsubscribe':
    ...
    */
    apply(name, args) {
        ...
        this.output.onNext({key, name, args: convertedArgs});
        return ret.flatMap(obsOrVal => {
            if (typeof obsOrVal.subscribe === 'function') {
                return obsOrVal;
            } else {
                return Observable.just(obsOrVal);
            }
        });
    }
    ...
    */
    couple(other) {
        this.output.forEach(o => other.input.onNext(o));
        other.output.forEach(o => this.input.onNext(o));
    }
    ...
    convert(obj) {
        return _.cloneDeepWith(obj, o => {
            if (o && typeof o.subscribe == 'function') {
                // Observable
                const key = uuid();
                this.observables[key] = o.doOnCompleted(() => {
    ...
    linkObservable(id) {
        return Observable.create(observer => {
            ...
            this.output.onNext({$$rxrpc: ['subscribe', id]});

            // When user disposes of stream
            return () => {
                this.output.onNext({$$rxrpc: ['unsubscribe', id]});
